...
| **P3 无服务器 + 集成** | 9/9(水) – 9/14(月)<br>*提前 3 天启动* | ✅ Lambda + API Gateway — 测验 **10/10** 🎯<br>🔸 SQS/SNS/EventBridge/Step Functions 内容已讲完（笔记 `11-sqs-sns-eventbridge-stepfunctions.md`，**12 题测验待做**）<br>Kinesis 三兄弟（KDS vs Firehose 已在错题 10 覆盖） | Week 7 |
...
1. **做 `notes/saa/11-sqs-sns-eventbridge-stepfunctions.md` 末尾的 12 题测验**
...
**9/10-9/11 完成**：SQS / SNS / EventBridge / Step Functions 内容已讲完（12 题测验待做）
...
- [~] SQS：标准 vs FIFO、可见性超时、长轮询、死信队列（DLQ）（笔记已写，测验待做）
- [~] SNS：扇出模式（Fan-out）+ 消息过滤（笔记已写，测验待做）
- [~] EventBridge（SaaS/cron/Replay）、Step Functions（Standard vs Express）（笔记已写，测验待做）
