...
| **P5 容器 + IaC + 成本** | 9/13(日) – 9/14(月)<br>*提前 4 天* | ✅ 容器三兄弟 + ECR + App Runner — 测验 **10/10** 🎯<br>🔸 CloudFormation / Beanstalk / 成本优化内容已讲完（笔记 `17-iac-cost.md`，**12 题测验待做**） | Week 9 |
...
1. **做 `notes/saa/17-iac-cost.md` 末尾的 12 题测验** → **P5 收工 = 全部知识模块结束**
2. 进入 **P6 模拟题冲刺**（原定 9/19 开始，可提前）：
   - 模拟卷① + 错题精读 → 写 cheatsheets → 模拟卷② → 薄弱域专项 → 模拟卷③
   - 富余时间可加到 **4-5 套**
...
**9/14 完成**：CloudFormation / Beanstalk / 成本优化内容已讲完（12 题测验待做）
...
- [~] CloudFormation：模板结构、StackSets、Change Sets、Drift Detection（笔记已写，测验待做）
- [~] Elastic Beanstalk：五种部署策略（笔记已写，测验待做）
- [~] 成本优化：RI vs Savings Plans vs Spot、Cost Explorer、Budgets（笔记已写，测验待做）
