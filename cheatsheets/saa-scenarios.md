...
### 🔴 多选题（Select TWO/THREE）—— 先判断是哪一种

> **踩过的坑：默认所有多选题都是「互补型」，于是在同族选项里只敢选一个。**

| 类型 | 结构 | 特征 | 例 |
|---|---|---|---|
| **A 互补型** | 两个答案拼成**一个**完整方案 | 选项是**不同类型的动作** | Multi-AZ（高可用）+ DMS（迁移）<br>私有子网（隔离）+ SG 引用（限来源） |
| **B 并列型** | 两个**各自独立可行**的方案 | 选项是**同类动作的不同变体** | S3→Standard-IA ／ S3→Glacier<br>（都是"转存储类"） |
...
① 数题干有几个【独立要求】
     ≥2 个  → 互补型，一个答案对应一个要求
      1 个  → 并列型，找出所有可行实现
② 交叉验证选项动作类型
     不同类动作（建库/迁移、网络/权限）→ 互补型
     同类动作的变体（转IA/转Glacier）  → 并列型
```

> ⚠️ **多选必须全对才得分，选对一半 = 0 分。**
> 先用排除法砍掉「虚构能力」和「明显不对症」的，剩下的再按上面分类配对。
