...
### ⭐ 6.5 客户端加密（CSE）—— 原笔记缺失（TD Test 1 Q45 错题）

**五种加密方式，按「什么被发给 AWS」排列：**

| 方式 | **未加密数据**发给 AWS？ | **密钥**发给 AWS？ | 谁做加密 |
|---|---|---|---|
| **SSE-S3** | ✅ 是 | AWS 自己的密钥 | S3 |
| **SSE-KMS** | ✅ 是 | 密钥在 KMS（AWS 保管） | S3 |
| **SSE-C** | ✅ 是 | ⚠️ **是**（随请求发，用完丢弃） | S3 |
| **CSE-KMS** | ❌ 否 | ⚠️ **KeyId 发给 AWS** | 你的客户端 |
| ⭐ **CSE + 客户端主密钥** | ❌ **否** | ❌ **否** | 你的客户端 |

**🔑 最容易混的一对：SSE-C vs CSE（客户端主密钥）**

| | **SSE-C** | **CSE + 客户端主密钥** |
|---|---|---|
| 加密在哪做 | **AWS 服务端** | ⭐ **你的客户端** |
| 上传时数据状态 | ⚠️ **明文** | ⭐ **已是密文** |
| 密钥传输 | ⚠️ **随每次请求发给 AWS** | ⭐ **从不离开你** |
| AWS 能接触明文吗 | ⚠️ 能（加密瞬间） | ❌ 完全不能 |

**判据：**

| 题干 | 选 |
|---|---|
| ⭐ **`unencrypted data should never be sent to AWS`** / 上传前必须加密 | **客户端加密 CSE** |
| ⭐ **`master keys should never be sent to AWS`** / 最严格合规 | ⭐ **CSE + 客户端主密钥** |
| 自己管密钥，但可以让 S3 做加密 | **SSE-C** |
| 要审计密钥使用 / 控制密钥策略 | **SSE-KMS** |

> 🔴 两个 "never" 各排除一批：`unencrypted data never` 砍掉所有 SSE；
> `master key never` 再砍掉 CSE-KMS（KeyId 要发给 AWS）→ 只剩客户端主密钥。
