...
| **⚠️ 只能启动时指定** | **不能给已运行的实例事后挂载 instance store**（选项写 "attach instance store to your existing instance" = 不可能操作） |
...
> ⚠️ **EBS 快照 ≠ 你 bucket 里的 S3 对象**（高频诱饵）
>
> | | EBS 快照 | S3 对象 |
> |---|---|---|
> | 存在哪 | S3（**AWS 内部管理，你看不到**） | 你的 bucket |
> | 能用 S3 生命周期策略管吗 | ❌ **不行** | ✅ 可以 |
> | 归档 | **EBS Snapshot Archive**（独立功能） | 生命周期转 Glacier |
> | 自动化创建/删除 | **DLM**（Data Lifecycle Manager） | S3 生命周期策略 |
>
> 题干提到"已配置 EBS 快照保留规则"时，**通常是干扰句**——它和后面要求的
> "备份数据存对象存储 + 转归档"是两件独立的事。
